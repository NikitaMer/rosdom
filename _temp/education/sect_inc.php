...
if(isset($_REQUEST['SECTION_CODE'])){
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => 17, 'CODE' => $_REQUEST['SECTION_CODE'])); 
	$section = $res->Fetch(); 
	unset($section_id);
	...
	//echo $parent_id;
	$subsections=CIBlockSection::GetList(Array("NAME"=>"ASC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>"17", "DEPTH_LEVEL"=>"2", "SECTION_ID"=>$parent_id));
};
...
$ar_result=CIBlockSection::GetList(Array("NAME"=>"ASC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>"17", "DEPTH_LEVEL"=>"1"));
?>
...
if (($res["ID"] != $section_id) OR (!empty($_REQUEST['ELEMENT_CODE']))) echo '<a href="/education/'.$res['CODE'].'/">';
echo $res["NAME"];
...
if (($res["ID"] == $parent_id) && (($res['RIGHT_MARGIN'] - $res['LEFT_MARGIN']) > 1)) {
	...
	while($res_sub=$subsections->GetNext()){
		echo '<li>';
		if ($res_sub["ID"] != $section_id) echo '<a href="/education/'.$res['CODE'].'/'.$res_sub['CODE'].'/">';
		echo $res_sub["NAME"];
