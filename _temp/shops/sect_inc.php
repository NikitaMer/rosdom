...
if(isset($_REQUEST['SECTION_CODE'])){
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => 22, 'CODE' => $_REQUEST['SECTION_CODE'])); 
	$section = $res->Fetch(); 
	unset($section_id);
	...
	$dbSect = CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>$section["IBLOCK_ID"], "<=LEFT_BORDER" => $section["LEFT_MARGIN"], ">=RIGHT_BORDER" => $section["RIGHT_MARGIN"], "DEPTH_LEVEL"=>"1"), false);
	if($res = $dbSect->GetNext()) $parent_id = $res["ID"];
	//if(count($ar_result) != 0) echo '!!!<Br>';
	//echo $parent_id;
	$subsections=CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>"22", "DEPTH_LEVEL"=>"2", "SECTION_ID"=>$parent_id), false, Array('UF_MENUTITLE'));
};
...
$ar_result=CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>"22", "DEPTH_LEVEL"=>"1"), false, Array("UF_MENUTITLE"));
?>
...
if (($res["ID"] != $section_id) OR (!empty($_REQUEST['ELEMENT_CODE']))) echo '<a href="/shops/'.$res['CODE'].'/">';
...
if (($res["ID"] == $parent_id) && (($res['RIGHT_MARGIN'] - $res['LEFT_MARGIN']) > 1)) {
	...
	while($res_sub=$subsections->GetNext()){
		echo '<li>';
		if ($res_sub["ID"] != $section_id) echo '<a href="/shops/'.$res['CODE'].'/'.$res_sub['CODE'].'/">';
