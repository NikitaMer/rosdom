...
<form method="post" action="<?=POST_FORM_ACTION_URI?>" name="form_photo" id="form_photo" class="photo-form">
	...
	<tr class="photo-album-edit-cont">
		<td class="photo-al-edit-icon-sect photo-album-thumbs-list">
			...
			<noindex>
<!-- 				<a rel="nofollow" href="<?=$arResult["SECTION"]["EDIT_ICON_LINK"]?>" 
	    onclick="BX.PreventDefault(event); EditAlbum('<?= addslashes(CUtil::JSEscape($arResult["SECTION"]["EDIT_ICON_LINK"]))?>'); return false;">
<span><?=GetMessage("P_SECTION_EDIT_ICON")?></span></a> -->
				<?$icon_link = "section_edit_icon.php?SECTION_ID=".$arResult["SECTION"]["ID"]."&ACTION=edit";?>
...
while ( $block = $db->GetNext()) {
	...
	if($block['ID'] == $arResult["SECTION"]["USER_FIELDS"]["UF_COMPANY"]["VALUE"]) $firms[$block['ID']]['SELECTED'] = 'Y';
}
//echo "<pre>!"; print_r($arResult); echo "</pre>";?>
...
foreach ($firms as $firm) {
	if ($firm['SELECTED'] == 'Y') $sel = 'selected';
	echo "<option value=".$firm["ID"]." ".$sel.">".$firm["NAME"]."</option>";
}
?>
<option value="self" <?if(!$sel) echo 'selected';?>>Оставить без привязки к фирме</option>
</select></td></tr>
