...
<?
		...
		case "WORK_NOTES":
			...
			break;
		case "PERSONAL_PHONE":
			?>+7 <input size="15" size="10"  id="phone" name="REGISTER[<?=$FIELD?>]" value=<?=$arResult["VALUES"][$FIELD]?>><?
			break;
		default:
...
?>
		<tr>
			<td></td>
		<td><input type="checkbox" onClick="agree();">
<script>
function agree(){
//alert("!"); 
var status=$('#submit').attr('disabled'); 
if (status == "disabled") $('#submit').removeAttr('disabled');
else $('#submit').attr('disabled','disabled');
//alert(status);
}
</script> 
		я подтверждаю согласие с <a href="/about/agreement/" target="_blank">"ѕользовательским соглашением"</a>
		</td>
		</tr>
	</tbody>
	<tfoot>
		<tr>
			<td></td>
			<td><input type="submit" id="submit" name="register_submit_button" value="<?=GetMessage("AUTH_REGISTER")?>" disabled/></td>
		</tr>
	</tfoot>
</table>
