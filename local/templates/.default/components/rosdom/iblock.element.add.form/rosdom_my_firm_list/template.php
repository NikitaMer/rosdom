...
$('#dat').mask('99/99/9999');
$('#product').mask('aaaaa-€€€€€-99999-*****');
});</script>

<h1>–едактирование элемента</h1>
<?
...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<table class="data-table">
					...
					<td width="70%">
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?> 
								<?if($propertyID ==54 || $propertyID==55) echo "+7 "?>
								<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"  value="<?=$value?>" <?if($propertyID ==54 || $propertyID==55) echo "class='phone' size='11'"; else echo "size='20'"?> /><br /><?
		...
		<tfoot>
			<tr>
				<td colspan=2>
<span class="starrequired">*</span> ѕол€, помеченные звездочкой, об€зательны дл€ заполнени€. <br /><br />
				</td>
			</tr>
			<tr>
				<td>
				...
				<td >
					<input id="iblock_add" style="border:0; bachground:white; text-decoration:underline; color:#0479bd; cursor:pointer; " type="submit" name="iblock_submit" value="ѕродолжить" />
				</td>
			</tr>
		</tfoot>
	</table>
