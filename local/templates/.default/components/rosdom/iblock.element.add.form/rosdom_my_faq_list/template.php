...
<h1>Шаг 1. Редактирование "Вопрос и Ответ" </h1>
<p>
</p>
...
$arResult['PROPERTY_LIST_FULL']['NAME']['PROPERTY_TYPE'] = 'T';
$arResult['PROPERTY_LIST_FULL']['NAME']['ROW_COUNT'] = 8;
$arResult['PROPERTY_LIST_FULL']['NAME']['COL_COUNT'] = 50;
	...
	<table class="data-table">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<tr>
					<td width="30%"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></td>
					...
					<td width="70%">
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=($propertyID=="PREVIEW_TEXT")?$Question:$value?></textarea>
...
<input type="hidden" name="PROPERTY[PREVIEW_TEXT][<?=$i?>]" size="25" value="<?=$value?>" /><br />
		...
		<tfoot>
			<tr>
				<td>
				<a href="<?=$APPLICATION->GetCurDir();?>"><?=GetMessage("IBLOCK_FORM_BACK")?></a>
				</td>
				<td >
					<input id="iblock_add" style="border:0; bachground:white; text-decoration:underline; color:#0479bd; cursor:pointer; " type="submit" name="iblock_submit" value="Продолжить редактирование" />
