...
if(isset($IPROPERTY["ELEMENT_META_DESCRIPTION"]) && $IPROPERTY["ELEMENT_META_DESCRIPTION"] != ''){
    $des = $IPROPERTY["ELEMENT_META_DESCRIPTION"];    
}else{
    $des = GetMessage("HOUSE_PROJECT").$arResult["VARIABLES"]["ELEMENT_CODE"];
}                                                                                               
$description = str_replace("&gt;", "&gt; ", $des);         
$APPLICATION->SetPageProperty("description", strip_tags(html_entity_decode($description)));
unset($basketAction);
/*if ($ElementID > 0)
{
